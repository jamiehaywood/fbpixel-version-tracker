...
if (localSourceHash !== remoteSourceHash) {
  const formattedFbPixlSource = prettier.format(fbPixelSource, {
    parser: 'babel',
  })

  await fs.writeFile('hash.txt', remoteSourceHash, 'utf-8')
  await fs.writeFile('fb-pixel-source.js', formattedFbPixlSource, 'utf-8')
  ...
  await octokit.repos.createOrUpdateFileContents({
    ...
    content: Buffer.from(formattedFbPixlSource).toString('base64'),
